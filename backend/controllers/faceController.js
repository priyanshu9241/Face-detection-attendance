...
async function checkFace(req, res) {
  ...
  try {
    ...
    res.json({
      msg: result[0] === undefined ? { _label: "no data" } : result[0],
    });
  } catch (error) {
...
async function getDescriptorsFromDB(imagePath) {
  try {
    ...
    const labeledDescriptors = faces.map((face) => {
      ...
      return new faceapi.LabeledFaceDescriptors(face.studentId, [
        new Float32Array(faceDescriptorValues),
      ]);
    });
